...
import org.gradle.internal.resource.transport.httpProxy.*;
...
public class ProxyPluginResolutionServiceClient implements PluginResolutionServiceClient {
    ...
    private ProxyResourceAccessor resourceAccessor = null;

    public ProxyPluginResolutionServiceClient(SslContextFactory sslContextFactory) {
        this(sslContextFactory, null);
    }

    public ProxyPluginResolutionServiceClient(SslContextFactory sslContextFactory, ProxyResourceAccessor resourceAccessor) {
    ...
    private <T> Response<T> request(final String requestUrl, final Class<T> type, final Action<? super T> validator) {
        ...
        try {
            ProxyResponseResource response = getResourceAccessor().getRawResource(requestUri);
            try {
    ...
    private ProxyResourceAccessor getResourceAccessor() {
        if (resourceAccessor == null) {
            resourceAccessor = new ProxyResourceAccessor(new ProxyClientHelper(new DefaultHttpSettings(Collections.<Authentication>emptyList(), sslContextFactory)));
        }
